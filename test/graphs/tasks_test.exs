defmodule Gannbaruzoi.TasksTest do
  ...
  describe "query tasks" do
    ...
    test "returns all tasks", %{document: document, user: user} do
      ...
      assert {:ok, %{data: %{"tasks" => %{"edges" => [%{"node" => task}]}}}} = result
    ...
    @tag login_as: "user@email.com"
    test "returns tasks which loggedSize is 0", %{document: document, user: user} do
      insert!(:task, %{user: user})
      result = execute_query(document, context: %{current_user: user})
    ...
    @tag login_as: "user@email.com"
    test "returns tasks which loggedSize is 1", %{document: document, user: user} do
    ...
    @tag login_as: "user@email.com"
    test "returns tasks which loggedSize is still 0", %{document: document, user: user} do
      inserted_task = insert!(:task, %{user: user})
  ...
  describe "mutation createTask" do
    ...
    @tag login_as: "user@email.com"
    test "returns new task with valid args", %{document: document, user: user} do
      variables = %{
        "clientMutationId" => "1",
      ...
      assert ~w(description estimatedSize id parentId status type) == Map.keys(task)
    end

    @tag login_as: "user@email.com"
    test "returns new subtask with valid args", %{document: document, user: user} do
    ...
    @tag login_as: "user@email.com"
    test "fails to create with invalid args", %{document: document, user: user} do
      variables = %{
        "clientMutationId" => "1",
        "description" => "",
        "estimatedSize" => 5
  ...
  describe "mutation updateTask" do
    ...
    @tag login_as: "user@email.com"
    test "fails to update task with invalid args", %{document: document, user: user} do
      task = insert!(:task, %{user: user})
  ...
  describe "mutation deleteTask" do
    ...
    @tag login_as: "user@email.com"
    test "fails to delete task with invalid args", %{document: document, user: user} do
